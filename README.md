...
## FYI
This implementation is still not perfect. There is still a small possibility in L1 that the validator that wants to manipulate the result is randomly selected to do it in the last block, but it is unpredictable so the possibility is very low. At the current state of most of the L2s, you're trusting a single relayer. But this implementation solves definitely manipulation and predictability from participants and L1 validators. There are tons of possibilities to improve with more tech like ZKVM, based relayer, and 0 lag to read L1 blocks from L2s. I'll update in the future to reach full security, but we're on the right track to stop trust oracles.
