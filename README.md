...
## FYI
This implementation is still not perfect, there is still a small possibility in L1 that the validator that wants to manipulate the result is randomly selected to do it in the last block, but is unpredictable so the possibility is very low. In L2 still you're trusting the relayer at the current state of most of L2. But this implementation solves definitley manipulation and predictability from participants and L1 Validators. There are tons of possibility to improve with more tech like ZKVM, Based Relayer and 0 lag to read L1 block from L2s. I'll update in the future to reach full security, but we're on the right track to stop trust oracles.
